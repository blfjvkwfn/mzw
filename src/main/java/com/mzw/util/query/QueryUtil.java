...
public class QueryUtil<T> {
    ...
    public Pageable pageable(QueryParam queryParam) {
        ...
        return PageRequest.of(queryParam.getPage() > 1 ? queryParam.getPage() -1 : 0,
                queryParam.getRows() <= 0 ? 1 : queryParam.getRows(), org.springframework.data.domain.Sort.by(orders));
    }

    private void updateOrders(QueryParam queryParam, List<org.springframework.data.domain.Sort.Order> orders) {
        Sort sort = queryParam.getSort();
        if (EmptyUtil.isObjEmpty(sort) || !queryParam.isPagination()) {
            return;
        }

        if (Sort.SORT_TYPE_ASC.equals(sort.getOrder())) {
            orders.add(new org.springframework.data.domain.Sort.Order(org.springframework.data.domain.Sort.Direction.ASC, sort.getName()));
        } else if (Sort.SORT_TYPE_DESC.equals(sort.getOrder())) {
            orders.add(new org.springframework.data.domain.Sort.Order(org.springframework.data.domain.Sort.Direction.DESC, sort.getName()));
        }
    }
